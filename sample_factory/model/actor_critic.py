...
class ActorCriticSharedWeights(ActorCritic):
    def __init__(
        ...
        self.base_policies = ['skill_GraphPickUpSkill', 'skill_GraphPickUpFromTargetSkill', 'skill_GraphPutDownSkill', 'skill_GraphPutDownOnTargetSkill']
    ...
    def forward(
        ...
        if 'actions' in result:
            ...
            if current_policy == 'controller':
                ...
                base_action = torch.zeros(3, dtype=new_rnn_states.dtype, device=new_rnn_states.device)
                policy_choice = torch.tensor([0], dtype=new_rnn_states.dtype, device=new_rnn_states.device)
                action = torch.concat([base_action, torch.tensor([policy_choice], dtype=new_rnn_states.dtype, device=new_rnn_states.device)], dim=0)
